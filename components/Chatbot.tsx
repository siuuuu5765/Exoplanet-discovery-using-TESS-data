  ...
  useEffect(() => {
    ...
    const addInitialMessage = async () => {
        ...
        if (!chatSessionRef.current) {
            setMessages([{ id: Date.now(), sender: 'bot', text: 'AstroBot is offline. The application is missing its API key.' }]);
            setIsLoading(false);
            return;
        }
        try {
    ...
    <>
      ...
      >
        ...
        <form onSubmit={handleSendMessage} className="p-4 border-t border-space-light flex items-center gap-2">
          ...
          />
          <button type="submit" className="bg-accent-magenta text-white p-2 rounded-full disabled:opacity-50" disabled={isLoading || !chatSessionRef.current}>
